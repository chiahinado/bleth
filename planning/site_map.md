...
- 明るい配色と手描きイラストの質感を維持し、下層ページは細い罫線とセクションカラーのアクセントで統一する。
- 下層ページでは説明的なリード文や重複見出しを置かず、固有名と実データを中心に表示する。
...
- BLETHの楽曲と音源は公開可能とする。歌詞は掲載しない。第三者紹介文と個人情報を含む展示は別途確認する。
...
| `/music/` | 1 | 公開 | 楽曲一覧 |
| `/music/{slug}/` | 9 | 公開 | 各楽曲のクレジット、音源（歌詞は非掲載） |
| `/shows/` | 1 | 公開 | 旧公演案内4ページを年代別に統合 |
| `/extras/` | 1 | 休止中 | 4コンテンツを休止表示し、リンクは置かない |
| `/extras/quiz/` | 1 | 休止中 | 休止案内のみ |
| `/extras/fortune/` | 1 | 休止中 | 休止案内のみ |
| `/extras/haunted-house/` | 1 | 休止中 | 休止案内のみ |
| `/extras/copybands/` | 1 | 休止中 | 一覧上で休止表示のみ |
...
歌詞は全曲とも掲載しない。旧RealMedia音源3曲はMP3へ変換して掲載し、MIDI、携帯向け音源はそのまま再生対象にしない。
