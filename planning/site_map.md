...
- トップのホットスポットは四角い白枠を表示せず、対象アトラクションの形に沿った短い発光で操作対象を示す。
...
- 旧プロフィールで使われていた各メンバーの主写真を、メタデータを除去した公開用コピーとして掲載する。
- メンバー写真は共通の3:4枠、穏やかな共通色調、被写体ごとのトリミングで顔の大きさを揃える。
- メンバー紹介には名前、パート、音楽活動に関係する簡潔な紹介だけを掲載する。
...
個別URLは作らない。カードの内容が短い場合も、写真、名前、パート、簡潔な紹介で1ページ内に並べる。
